...
describe("#market #conditionalTokens #cpmmValidation", function()
  ...
  it("should have initial state", function()
    -- assert initial state
		assert.is.same("", CPMM.marketId)
    assert.is.same("", CPMM.incentives)
    assert.is.same("", CPMM.configurator)
    assert.is.same(false, CPMM.initialized)
    ...
    assert.is.same(tokens:new(), CPMM.tokens)
    assert.is.same(token:new(), CPMM.token)
    assert.is.same(0, CPMM.lpFee)
	end)

  it("should init", function()
    local notice = CPMM:init(
    ...
    -- assert state 
    assert.is.same(msgInit.Tags.MarketId, CPMM.marketId)
    assert.is.same(msgInit.Tags.Incentives, CPMM.incentives)
    assert.is.same(msgInit.Tags.Configurator, CPMM.configurator)
    assert.is.same(true, CPMM.initialized)
    assert.is.same({}, CPMM.poolBalances)
    assert.is.same({}, CPMM.withdrawnFees)
    assert.is.same('0', CPMM.feePoolWeight)
    assert.is.same('0', CPMM.totalWithdrawnFees)
    assert.is.same(tonumber(msgInit.Tags.LpFee), CPMM.lpFee)
    assert.is.same(msgInit.Tags.ConditionId, CPMM.tokens.conditionId)
    assert.is.same(positionIds, CPMM.tokens.positionIds)
    assert.is.same(msgInit.Tags.CollateralToken, CPMM.tokens.collateralToken)
    assert.is.same(msgInit.Tags.OutcomeSlotCount, CPMM.tokens.outcomeSlotCount)
    assert.is.same(tonumber(msgInit.Tags.CreatorFee), CPMM.tokens.creatorFee)
    assert.is.same(msgInit.Tags.CreatorFeeTarget, CPMM.tokens.creatorFeeTarget)
    assert.is.same(tonumber(msgInit.Tags.ProtocolFee), CPMM.tokens.protocolFee)
    assert.is.same(msgInit.Tags.ProtocolFeeTarget, CPMM.tokens.protocolFeeTarget)
    assert.is.same(msgInit.Tags.Name, CPMM.token.name)
    assert.is.same(msgInit.Tags.Ticker, CPMM.token.ticker)
    assert.is.same(msgInit.Tags.Logo, CPMM.token.logo)
    -- assert notice
    assert.are.same({
