...
local function isBuyOrderCompletion(msg)
  if msg.From == ConditionalTokens and msg.Action == "Split-Position-Notice" and msg.Tags["X-OutcomeIndex"] ~= "0" then
    return true
  else
    return false
  end
...
local function isSellOrderCompletion(msg)
  if msg.From == CollateralToken and msg.Action == "Credit-Notice" and msg["X-Action"] == "Merge-Positions-Completion" then
    return true
  else
    return false
  end
...
Handlers.add("SellOrderConditionalTokenCompletion", isSellOrderConditionalTokenCompletion, function(msg)
  ...
  -- Update Pool Balances
  PoolBalances = json.decode(msg.Tags.RemainingBalances)
