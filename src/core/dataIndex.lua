--[[
Process to facilitate complex queries required to optimise the frontend and data monitoring.
]]--
...
Version = "0.0.1"
Name = "DataIndex-v" .. Version

-- @dev used to reset state between integration tests
ResetState = true

-- @dev maintain or reset state as per the above flag
...
Handlers.add("ProtocolProcess.Update",
  ...
  function (msg)
    assert(msg.From == Admin, 'Sender must be admin!! ' .. Admin)
...
Handlers.add("Market.Draft-Update",
  ...
  function (msg)
    ...
    if marketCount > 0 then
      ...
      dbAdmin:exec(string.format([[
        UPDATE Markets SET type = "%s", price = "%s", category = "%s", start_timestamp = "%s", end_timestamp = "%s", image_url = "%s", condition_name = "%s", condition_detail = "%s", resolution_agent = "%s", timestamp = "%s" WHERE id = "%s";
      ]], market.type, market.price, market.category, start_timestamp, end_timestamp, market.image_url, market.condition_name, market.condition_detail, market.resolution_agent, msg.Timestamp, id))
...
Handlers.add("Market.Open-Update",
  ...
  function (msg)
    ...
    if marketCount > 0 then
      ...
      dbAdmin:exec(string.format([[
        UPDATE Markets SET type = "%s", price = "%s", category = "%s", start_timestamp = "%s", end_timestamp = "%s", image_url = "%s", condition_name = "%s", condition_path = "%s", condition_detail = "%s", timestamp = "%s" WHERE id = "%s";
      ]],market.type, market.price, market.category, market.start_timestamp, market.end_timestamp, market.image_url, market.condition_name, market.condition_path, market.condition_detail, msg.Timestamp, market.id))
...
function (msg)
  ...
  dbAdmin:exec(string.format([[
    UPDATE Markets SET price = "%s", timestamp = "%s" WHERE id = "%s";
  ]], market.price, msg.Timestamp, market.id))
  ...
  local userdata = utils.map(
    ...
    dbAdmin:exec(string.format([[
      ...
      WHERE 
          u.id = "%s"
      GROUP BY 
          u.id, u.nickname, u.banned, u.registered_ts, u.last_claim_posix, u.last_active_ts, u.timestamp;
      ]], user.id)
    )
  )[1]
