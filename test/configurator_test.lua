...
-- _G.msg.reply = function()
--   print("here")
-- end -- mock reply function

local process = require "configurator" -- require so that process handlers are loaded
...
describe("greetings", function()
  ...
  it("should have no LastGreeting", function()
    assert.are.equal(_G.Name, 'Configurator')
  end)

  it("should get info", function()
    local info = ao.send({ Target = ao.id, Action = "Info" })
    assert.are.equal(info.Admin, 1)
  end)
