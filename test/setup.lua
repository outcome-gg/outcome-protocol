...
local function mockedRequire(moduleName)
  ...
  if moduleName == ".crypto" then
    return originalRequire("test.mocked-env.lib.json")
  end

  if moduleName == "modules.config" then
    return originalRequire("src.configurator.modules.config")
  end

  if moduleName == "modules.configurator" then
    return originalRequire("src.configurator.modules.configurator")
  end
